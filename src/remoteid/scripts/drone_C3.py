...
class Drone:
    def __init__(self):
        self.posx = 50*1000 + (-470886695)
        self.posy = 50*1000 + (-225911104)
        self.posz = 50.0 + (30)
        ...
        rospy.Subscriber("drone3_command", Drone_command, callback, self)

    def broadcast(self):
        pub = rospy.Publisher('drone3_info_encoded', Drone_data_encoded, queue_size=10)
        ...
        msg_encoded.UAS_ID = "PR-202200002".zfill(20)
        ...
        while not rospy.is_shutdown():
            ...
            if(posx >= 11.999 or posx <= 0):
                velx *= -1

            if(posy >= 11.999 or posy <= 0):
                vely *= -1

            if(posz >= 9.999 or posz <= -2):
                velz *= -1
